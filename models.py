...
def create_compiled_keras_vgg16_model():
    vgg16_conv = tf.keras.applications.VGG16(weights=pretrained, include_top=False, input_shape=input_shape)
    vgg16_conv.summary()
...
def create_compiled_keras_vgg19_model():
    ...
    vgg19_conv = tf.keras.applications.VGG19(weights=pretrained, include_top=False, input_shape=input_shape)
    if not transfer_learning and pretrained != None:
...
def create_compiled_keras_resnet50_model():
    resnet50_conv = tf.keras.applications.ResNet50(weights=pretrained, include_top=False, input_shape=input_shape)
    if not transfer_learning and pretrained != None:
...
def create_compiled_keras_inception_v3_model():
    ...
    inception_v3_conv = tf.keras.applications.InceptionV3(weights=pretrained, include_top=False, input_shape=input_shape)
    if not transfer_learning and pretrained != None:
...
if __name__ == "__main__":
    ...
    for i, correct in enumerate(correct[:9]):
        plt.subplot(3,3,i+1)
        plt.imshow(X_test[correct].reshape(img_rows, img_cols, img_channels), cmap='gray', interpolation='none')
        plt.title("Predicted {}, Class {}".format(predicted_classes[correct], y_true[correct]))
        plt.tight_layout()
    plt.savefig("{}_{}_{}_{}_img{}x{}_correct_prediction.pdf".format(
        cnn_type, pretrained, transfer_learning, dataset_type, img_rows, img_cols
    ))
    plt.close()
    ...
    for i, incorrect in enumerate(incorrect[0:9]):
        plt.subplot(3,3,i+1)
        plt.imshow(X_test[incorrect].reshape(img_rows, img_cols, img_channels), cmap='gray', interpolation='none')
        plt.title("Predicted {}, Class {}".format(predicted_classes[incorrect], y_true[incorrect]))
        plt.tight_layout()
    plt.savefig("{}_{}_{}_{}_img{}x{}_incorrect_prediction.pdf".format(
        cnn_type, pretrained, transfer_learning, dataset_type, img_rows, img_cols
    ))
    plt.close()
